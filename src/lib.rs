...
impl ChipState {
	...
	pub fn generate(&mut self, buffer: &mut [f32], mut request_callback: impl FnMut(&mut ChipState, usize)) -> core::result::Result<(), String> {
		...
		for sample in (0..samples).step_by(2) {
			if self.wait_samples > 0.0 {
				...
				request_callback(self, self.command_buffer.len());
			}
	...
	fn execute_commands(&mut self) -> core::result::Result<(), String> {
		loop {
